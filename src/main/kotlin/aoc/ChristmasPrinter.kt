...
$BORING_VERTICAL_BORDER                   $RED/´   `\$RESET                  $BORING_VERTICAL_BORDER"""
  ...
  fun printActualPuzzleSolution(result: Any) {
    ...
    println(
      "$BORING_VERTICAL_BORDER${GREEN}Actual solution$RESET: $paddedResult$BORING_VERTICAL_BORDER"
    )
  }

  fun printPartHeader(partNo: Int) =
    println(
      "$BORING_VERTICAL_BORDER$christmasBorder$BRIGHT_GREEN Part $partNo $RESET$christmasBorder$BORING_VERTICAL_BORDER"
    )

  fun printDayHeader(dayNo: Int) {
    val paddedDay = dayNo.toString().padStart(2, '0')
    println(
      "$BORING_SECTION_NODE_LEFT$boringHorizontalBorder$BRIGHT_YELLOW*$RESET Day $paddedDay $BRIGHT_YELLOW*$boringHorizontalBorder$BORING_SECTION_NODE_RIGHT$RESET"
    )
  }

  fun printChristmasAsciiArt() =
    println(
      asciiHeaders[(0..2).random()] +
        "\n" +
        BORING_VERTICAL_BORDER +
        " ".repeat(44) +
        BORING_VERTICAL_BORDER
    )
